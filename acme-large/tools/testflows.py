    ...
    def parse(self, lab):
        ...
        for line in flows:
            if 'Failed' in line and line[0].isdigit():
                ...
                parsed_action = [ActiveEndpoint(word, lab) for word in self.split_strip(actions, ',')]
                ...
                from_point = ActiveEndpoint(from_device, lab)
                to_device, flow = self.split_strip(to_via, 'via')
                to_point = ActiveEndpoint(to_device, lab)
